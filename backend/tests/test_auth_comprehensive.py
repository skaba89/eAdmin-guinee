...
class TestChangePassword:
    ...
    async def test_change_password_weak_new_password(self, client: AsyncClient, auth_headers):
        response = await client.post(
            "/api/v1/auth/change-password",
            json={"current_password": "Test2026!", "new_password": "Short1!"},
            headers=auth_headers,
        )
        assert response.status_code == 422
        ...
        response = await client.post(
            "/api/v1/auth/change-password",
            json={"current_password": "Test2026!", "new_password": "nouveau2026!"},
            headers=auth_headers,
        )
        assert response.status_code == 422
        ...
        response = await client.post(
            "/api/v1/auth/change-password",
            json={"current_password": "Test2026!", "new_password": "NouveauPassword!"},
            headers=auth_headers,
        )
        assert response.status_code == 422
