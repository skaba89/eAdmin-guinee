...
intentionally deferred until the following migration creates
``courriers.institution_id``.
"""
...
def upgrade() -> None:
    ...
    # ``institution_id`` is added to courriers by add_tenants_and_institutions.
    # Enable RLS now; the comprehensive migration installs scoped policies once
    # that column exists.
