...
The historical Alembic graph started with RLS while assuming these tables
already existed. This baseline makes clean PostgreSQL provisioning deterministic
without changing revision identifiers already present on deployed databases.
"""
...
role_enum = sa.Enum("SUPER_ADMIN", "MINISTRE", "DIRECTEUR", "CHEF_SERVICE", "ADMIN", "AGENT", "MAIRIE", "AGENCE", "CITOYEN", name="roleenum")
document_status_enum = sa.Enum("DRAFT", "PENDING_REVIEW", "APPROVED", "ARCHIVED", "REJECTED", name="documentstatusenum")
workflow_status_enum = sa.Enum("DRAFT", "ACTIVE", "COMPLETED", "CANCELLED", name="workflowstatusenum")
workflow_step_status_enum = sa.Enum("PENDING", "IN_PROGRESS", "COMPLETED", "SKIPPED", "REJECTED", name="workflowstepstatusenum")
courrier_type_enum = sa.Enum("ENTRANT", "SORTANT", name="courriertypeenum")
courrier_priority_enum = sa.Enum("URGENT", "IMPORTANT", "NORMAL", "FAIBLE", name="courrierpriorityenum")
courrier_status_enum = sa.Enum("PENDING", "IN_PROGRESS", "TREATED", "ARCHIVED", name="courrierstatusenum")
...
def downgrade() -> None:
    ...
    op.drop_table("users")
    for enum_name in ("courrierstatusenum", "courrierpriorityenum", "courriertypeenum", "workflowstepstatusenum", "workflowstatusenum", "documentstatusenum", "roleenum"):
