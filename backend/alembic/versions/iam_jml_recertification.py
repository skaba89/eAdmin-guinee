...
def upgrade() -> None:
    ...
    for column in (
        ...
        op.create_index(
            f"ix_identity_lifecycle_events_{column}",
            "identity_lifecycle_events",
            [column],
        )
    ...
    for column in (
        ...
        op.create_index(
            f"ix_access_review_campaigns_{column}",
            "access_review_campaigns",
            [column],
        )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "campaign_id",
            "user_id",
            name="uq_access_review_campaign_user",
        ),
        sa.CheckConstraint(
    ...
    for column in ("campaign_id", "user_id", "decision", "decided_by"):
        op.create_index(
            f"ix_access_review_items_{column}",
            "access_review_items",
            [column],
        )
    ...
    # tenant on an item row because the item intentionally stores only an
    ...
    op.execute(
        """
        CREATE POLICY access_review_items_scoped_insert
        ON access_review_items FOR INSERT
        WITH CHECK (
            EXISTS (
                SELECT 1 FROM access_review_campaigns c
                WHERE c.id = access_review_items.campaign_id
    ...
    op.execute(
        """
        CREATE POLICY access_review_items_scoped_update
        ON access_review_items FOR UPDATE
        USING (
            EXISTS (
                SELECT 1 FROM access_review_campaigns c
                WHERE c.id = access_review_items.campaign_id
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP POLICY IF EXISTS identity_lifecycle_scoped_insert "
        "ON identity_lifecycle_events"
    )
    op.execute(
        "DROP POLICY IF EXISTS identity_lifecycle_scoped_read "
        "ON identity_lifecycle_events"
    )
    ...
    for column in (
        ...
        op.drop_index(
            f"ix_identity_lifecycle_events_{column}",
            table_name="identity_lifecycle_events",
        )
